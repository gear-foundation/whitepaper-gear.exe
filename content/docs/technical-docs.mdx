...
description: Vara.eth Technical Documentation
---

<Callout title="Document Info">
    **Authors:** Eugene Way
...
<figure>
    ![Component overview illustration](/img/architecture-overview.png)

    <figcaption>Component overview showing Router, Mirror, validators, Middleware, and WASM programs</figcaption>
</figure>

### 3.0 Gear Program (Core, Vara.eth)
...
<figure>
    ![Program message processing diagram](/img/program-message-processing.png)

    <figcaption>
        User sends message → Mirror queues → validators execute → Router commits batch → Mirror applies state transition
    </figcaption>
</figure>
...
<figure>
    ![Authority flow diagram](/img/autority-flow.png)

    <figcaption>
        Stakers → Vaults → Operators → Validators → Router (era activation) → Batch signing
    </figcaption>
</figure>
...
<figure>
    ![Finality flow diagram](/img/finality-flow.png)

    <figcaption>
        Validators build batch → FROST signatures → Router verifies → Apply transitions → Emit finality events
    </figcaption>
</figure>
...
<figure>
    ![Pre-confirmation flow diagram](/img/pre-confirmation.png)

    <figcaption>
        User sends message → validators execute immediately → Sign with FROST → SDK fetches &amp; verifies → Later:
        batch commits on L1
    </figcaption>
</figure>
...
<figure>
    ![Gas reverse flow diagram](/img/gas-reverse.png)

    <figcaption>
        User sends message (pays ETH) → Mirror pulls wVARA to Router → validators observe balance → Execute program →
        Consume executable balance → Distribute to validators
    </figcaption>
</figure>
